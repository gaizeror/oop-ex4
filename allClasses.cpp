...
GoldFish::GoldFish() : m_avgWeight(0) m_avgWeight(0) {};//set the default color to GRAY and other params to 0
...
Mermaid::Mermaid( const char* color, int childs, float avgLifetime, float preg, float milk, int fin, int gills, const char* firstName, const char* lastName )
{
    ...
    m_finCount = fin;
    m_gillsCount = gills;
    std::strcpy(m_firstName, firstName);
    std::strcpy(m_lastName, lastName);
...
Zoo::Zoo( const char* name, const char* address, float ticket, const char* open, const char* close )
{
    ...
    std::strcpy(m_openHours, open);
    std::strcpy(m_closeHours, close);
};
